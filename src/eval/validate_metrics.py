...
class MetricsValidator:
    ...
    def validate_metrics(self, metrics: Dict[str, float]) -> bool:
        ...
        # Check bounds (now safe because non-numeric values are already flagged)
        ...
        all_valid = all(
            [finiteness_valid, bounds_valid, relationships_valid, special_valid]
        )
    ...
    def _check_bounds(self, metrics: Dict[str, float]) -> bool:
        ...
        for metric, value in metrics.items():
            ...
            if not (min_val <= value <= max_val):
                msg = (
                    f"{metric} out of bounds: {value:.4f} not in [{min_val}, {max_val}]"
                )
                    ...
                    if range_size > 0:  # Avoid division by zero
                        if abs(value - min_val) < 0.1 * range_size:
                            self.warnings.append(
                                f"{metric} near lower bound: {value:.4f}"
                            )
                        elif abs(value - max_val) < 0.1 * range_size:
                            self.warnings.append(
                                f"{metric} near upper bound: {value:.4f}"
                            )
    ...
    def _check_relationships(self, metrics: Dict[str, float]) -> bool:
        ...
        for relationship in self.RELATIONSHIPS:
            ...
            if rel_type == "ratio_abs_denom":
                ...
                if (
                    abs(denom_val) > 1e-10
                ):  # Changed from denom_val > 1e-10 to handle potential small negative values
                    computed = num_val / denom_val
                else:
                    # Near-zero denominator makes ratio undefined
                    msg = (
                        f"{dependent} ratio undefined: denominator {denominator} "
